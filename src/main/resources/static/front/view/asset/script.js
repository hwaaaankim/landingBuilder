...
(function ($) {
  ...
  function initCommonLogin() {
    ...
    $('[href^="' + loginUrlPrefix + '"]').on("click", function () {
      ...
      switch (key) {
        case "login":
          location.replace("/view/member/login");
          break;
        case "mypage":
          location.replace("/view/member/mypage");
          break;
        case "confirm":
          location.replace("/view/member/order-history");
          break;
        case "reserve":
          location.replace("/view/member/point");
          break;
        case "favorite":
          location.replace("/view/member/favorite");
          break;
        case "member":
          location.replace("/view/member/member-edit");
          break;
        default:
          break;
      }
    ...
    $('[href="' + orderUrl + '"]').on("click", function () {
      $.ajax({
        ...
        if (data.action === "login") {
          var opt =
            "&ssl_tempid=" +
            $("#makeshop-form-common-order input[name=ssl_tempid]").val();
  ...
  function initCommonSearchForm() {
    ...
    $('[href="' + searchUrl + '"]').on("click", function () {
      ...
      var keyword = getFormValue(
        keywordId,
        count === $('[data-id="' + keywordId + '"]').length ? index : 0
      );
      var priceLow = getFormValue(
        priceLowId,
        count === $('[data-id="' + priceLowId + '"]').length ? index : 0
      );
      var priceHigh = getFormValue(
        priceHighId,
        count === $('[data-id="' + priceHighId + '"]').length ? index : 0
      );
      var category = getFormValue(
        categoryId,
        count === $('[data-id="' + categoryId + '"]').length ? index : 0
      );
      var originalCode = getFormValue(
        originalCodeId,
        count === $('[data-id="' + originalCodeId + '"]').length ? index : 0
      );
